...
eco_tables_abs <- function(long_or_wide = "wide"){
  ...
  dta_emp_state_sex<- readabs::read_abs("6202.0", tables = c("1","4","5",'6',"7","8","9","10a","11a") ) %>%
  ...
  dta_sex_dur_emp3 <-
    ...
    dplyr::mutate(
      ...
      unit = stringr::str_trim(unit, side = "both"),
      sex = stringr::str_trim(sex, side = "both"),
      duration = stringr::str_trim(duration, side = "both"),
      date = lubridate::as_date(date)
    ) %>%
    dplyr::filter(
      sex != ""
  ...
  return_list[[30]] = list("Empy by duration and gender 6291.0 tables 14b" ,dta_sex_dur_emp3 )
  ...
  #---------------------------------LAbour Force Age 
  dta_sex_age<- readabs::read_abs("6291.0.55.001", tables = c("1") )%>%
    dplyr::select(date,series,value,unit,series_type)
  
  dta_sex_age_col_1 <- data.frame( stringr::str_split_fixed(  dta_sex_age$series,pattern = ";",n = 7 ) )[,1:3] %>%
    dplyr::mutate(dplyr::across(where(is.character), ~ stringr::str_remove(.x,">")) ) %>%
    dplyr::mutate(dplyr::across(where(is.character), ~ stringr::str_remove(.x,">")) )
  
  dta_sex_age2 <-   dta_sex_age %>%
    dplyr::select(date,value,unit,series_type) %>%
    dplyr::bind_cols(dta_sex_age_col_1) %>%
    dplyr::rename(age = X1, employment_status = X2, sex = X3)
  ...
  return_list[[33]] = list("6291.0.55.001 table 1 Age sex status" ,dta_sex_age3 )
  
  #---------------------------------average weekly earnings 
  dta_avg_wages<- readabs::read_abs("6302.0", tables = c("2") )%>%
    dplyr::select(date,series,value,unit,series_type)
  
  
  dta_avg_wages_col_1 <- data.frame( stringr::str_split_fixed(  dta_avg_wages$series,pattern = ";",n = 7 ) )[,1:5] %>%
    dplyr::mutate(dplyr::across(where(is.character), ~ stringr::str_remove(.x,">")) ) %>%
    dplyr::mutate(dplyr::across(where(is.character), ~ stringr::str_remove(.x,">")) )
  
  dta_avg_wages2 <-   dta_avg_wages %>%
    dplyr::select(date,value,unit,series_type) %>%
    dplyr::bind_cols(dta_avg_wages_col_1) %>%
    dplyr::rename(earnings = X1, sex = X2, employment_status = X3,adult = X4,earning_type = X5)
  ...
  dta_avg_wages3 <-
    ...
    dplyr::mutate(
      ...
      unit = stringr::str_trim(unit, side = "both"),
      sex = stringr::str_trim(sex, side = "both"),
      adult = stringr::str_trim(adult, side = "both"),
      earning_type = stringr::str_trim(earning_type, side = "both"),
      date = lubridate::as_date(date)
    ) %>%
    dplyr::filter(
      sex != ""
  ...
  return_list[[34]] = list("6302.0 table 2 avg weekly wages" ,dta_avg_wages3 )
  ...
  return_list[[35]] = list("6302.0 table 2 avg weekly wages" ,dta_avg_wages3 )
  
  #--------------------------------STP
  stp <- 
    readabs::download_abs_data_cube(catalogue_string = "weekly-payroll-jobs-and-wages-australia", 
                                    cube = "6160055001_DO006") %>%
    ...
    mutate(subdivision = trimws(subdivision)) %>%
    mutate(subdivision = stringr::str_remove(subdivision, "[:upper:]-") ) %>%
    mutate(subdivision = gsub(x = subdivision,pattern = "\\&", replacement = "and") ) %>%
    mutate(subdivision = trimws(subdivision))%>%
