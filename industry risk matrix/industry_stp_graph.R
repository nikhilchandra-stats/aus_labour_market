...
stp_data_indus <- read.xlsx("stp_industry_2021-01-06.xlsx",sheet = 2,startRow = 6,detectDates = TRUE) %>%
...
stp_national <- read.xlsx("stp_national.xlsx",sheet = 2,startRow = 6,detectDates = TRUE) %>%
...
stp_data_gdp %>% 
  ggplot(aes(x = differene_covid, y = difference_since_covid) ) +
